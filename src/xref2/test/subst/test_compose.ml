...
let suite_name ~pp_id ~pp_path substs_desc =
  ...
  let pp =
    ...
    |> list ~sep:(unit " . ")
  in
  to_to_string pp substs_desc
  |> String.map (function '/' -> '-' | x -> x) (* Hack: Alcotest will make a file named like this *)
...
struct
  ...
  let test_all_subst ~make_local_path ~resolve ~pp_path ~subst_add substs_desc =
    ...
    and test_cases =
      List.fold_left (List.fold_left (fun acc (a, b) -> b :: a :: acc)) [] substs_desc
      |> List.sort_uniq compare
    in
    let make_test_case id =
      let test () =
        compare_sequential_and_composed ~resolve ~pp_path ~substs (make_local_path id)
      in
  ...
  let test_module =
    ...
    and subst_add s (a, b) =
      ...
      Subst.add_module (make_name a) b b s
    in
    test_all_subst ~make_local_path ~resolve ~pp_path ~subst_add

end

module Subst_type_expr =
struct
...
let () =
  Alcotest.run "Subst.compose" [
    ...
    All_subst.(test_module [ [ ("A", "B"); ("C", "D") ]; [ ("D", "E"); ("F", "C") ]; [ ("B", "F") ] ]);
    All_subst.(test_module [ [ ("A", "B") ]; [ ("A", "C") ] ]);

    Subst_type_expr.(test
        [
